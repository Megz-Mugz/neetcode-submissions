class Solution:
    def solve(self, board: List[List[str]]) -> None:
        ...
        def perform_dfs(row, col):
            ...
            if not (0 <= row < NROWS) or not (0 <= col < NCOsLS): return
        ...
        NROWS, NCOsLS = len(board), len(board[0])

        for row in range(NROWS):
            for col in range(NCOsLS):
                if (row == 0 or row == NROWS - 1) or (col == 0 or col == NCOsLS - 1):
                    perform_dfs(row, col)


        for r in range(NROWS):
            for c in range(NCOsLS):
