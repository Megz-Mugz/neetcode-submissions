    ...
    int findMin(vector<int> &nums) {
        ...
        while (left < right){
            ...
            if (nums[middle] > nums[right]){
                left = middle + 1; // don't want to visit middle again, since u already accounted for in on line 15.
            } else {
                // take first iteration of this: [5, 1, 2, 3, 4]
                // if middle is less than right, ur min value is closer to middle index
                right = middle - 1; // don't want to visit middle again, since u already accounted for in on line 15. 
            }
